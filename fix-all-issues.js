#!/usr/bin/env node /** * Comprehensive fix script for the job portal project */;
const fs = require('fs');
const path = require('path');
const {
  execSync
}
} = require('child_process') // Fix 1: Ensure all required directories existconst requiredDirs = [ 'backend/models'
  'backend/services',
  'backend/utils',
  'backend/config',
  'backend/middleware',
...
requiredDirs.forEach(dir => {
  ...
  }) // Fix 2: Create missing __init__.py files for Python packagesconst pythonPackages = [ 'backend';
  'backend/models',
  'backend/services',
  'backend/utils',
  'backend/config' ];
...
if (fs.existsSync(nextConfigPath)) {
  ;
  let nextConfig = fs.readFileSync(nextConfigPath, 'utf8') // Add error handling improvements;
}
  if (!nextConfig.includes('experimental: {')) {}
} // Fix 4: Ensure TypeScript configuration is optimalif (fs.existsSync('tsconfig.json')) {
  ;
  const tsconfig = JSON.parse(fs.readFileSync('tsconfig.json', 'utf8')) // Ensure we have good defaults;
...
if (!fs.existsSync('.gitignore')) {
  ...
  } else {} // Fix 6: Ensure environment example files existif (!fs.existsSync('.env.example') && fs.existsSync('.env.local')) {
...
} = require('child_process') // Check Next.jstry {
  const nextInfo = execSync('npx next info', { encoding: 'utf8'
}
});
  } catch (error) {} // Check TypeScripttry {
  ;
  execSync('npx tsc --noEmit --skipLibCheck', { stdio: 'pipe'
}
});
  } catch (error) {} // Check portstry {
  ;";
  const netstat = execSync('netstat -an | findstr "3000\\|8000"', { encoding: 'utf8'
}
}).trim();
  if (netstat) {} else {
} catch (error) {}`;
...
fs.writeFileSync('debug.js', debugScript) // Fix 9: Ensure backend has proper error handlingif (fs.existsSync('backend/main.py')) {
  let mainPy = fs.readFileSync('backend/main.py', 'utf8') // Check if it has proper error handling in startup;
  if (!mainPy.includes('try:') || !mainPy.includes('except')) {
}
} else {}
} // Fix 10: Summary and recommendations`;

fs.writeFileSync('fix-all.js', `${
  fs.readFileSync('health-check.js', 'utf8').replace('const issues = [];', 'const issues = []; let fixCount = 0;');
}
  }`) // Run the actual fixes// Ensure backend __init__.py files exist
const backendInits = [ 'backend/__init__.py',
