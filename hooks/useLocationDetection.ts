...
} from 'react' // Target countries configuration;
...
export function useLocationDetection(options: UseLocationDetectionOptions = {
  ...
  const [hasPermission, setHasPermission] = useState<boolean | null>(null) // Detect country from IP using multiple services;
  const detectCountryFromIP = useCallback(async (): Promise<Partial<LocationData> | null> => {
  ;
    const services = [ // Primary service;
      async () => {
...
} = position.coords // Reverse geocoding to get country info;
  ...
  ;
              resolve({
                ...
                coordinates: { la,t: latitude, lng: longitude }
})} else {
  ;
              resolve(null);
}
  ...
  }, [enableHighAccuracy]) // Main detection function;
  const detectLocation = useCallback(async (useGPS: boolean = false) => {
  ;
    setIsLoading(true);
    
    try {
      let locationData: Partial<LocationData> | null = null // Try GPS first if requested and permission available;
...
}, [hasPermission, detectLocationFromGPS, detectCountryFromIP, fallbackCountry]) // Manual country selection;
