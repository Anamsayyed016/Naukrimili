...
export function useMessages() {
  ...
  unreadOnly?: boolean; limit?: number; type?: string ;
}
  }) => {
  ;
    if (!session?.user) {
  ...
  ;
      ...
      const processedMessages = data.messages.map((message: Record<string, unknown>) => ({ ...message }
  ...
  ;
      ...
      if (response.ok) {
  ;
        const data = await response.json() // Refresh messages to get the latest state;
        await fetchMessages();
}
        return data.message}
} catch (err) {
