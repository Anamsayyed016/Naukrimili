...
export default function CompaniesPage() {
    ...
    try {
      ...
      if (data.success) {
        ...
        const transformedCompanies = data.companies.map((company: any) => ({
          ...
          reviews: Math.floor(Math.random() * 2000) + 100, // Generate realistic review counts
          logo: company.logo || `https://images.unsplash.com/photo-${Math.floor(Math.random() * 999999)}?w=100&h=100&fit=crop&crop=center`,
          description: company.description || 'Leading company in their industry',
          openJobs: Math.floor(Math.random() * 50) + 5, // Generate realistic job counts
          founded: company.founded?.toString() || '2010',
          headquarters: company.location || 'India',
          website: company.website || 'company.com',
  ...
  const loadEnhancedMockData = () => {
    const enhancedCompanies: Company[] = [
      // Tech Giants
      {
        id: '1',
        name: 'TechCorp India',
