...
export function CouponPreviewCard({ coupon }: { coupon: CouponPreviewData }) {
  ...
  return (
    <div className="rounded-xl border-2 border-indigo-100 bg-gradient-to-br from-white to-indigo-50/50 p-6 shadow-md">
      ...
      <div className="mt-4 pt-4 border-t border-indigo-100 space-y-3 text-sm">
        <div>
          <p className="text-gray-500 mb-1">Applicable Plans</p>
          <ul className="space-y-0.5">
            {plans.length === 0 ? (
              <li className="text-gray-400">Select plans</li>
            ) : (
              plans.map((p) => (
                <li key={p} className="flex items-center gap-1.5 text-gray-800">
        ...
        {coupon.minOrderAmount != null && coupon.minOrderAmount > 0 && (
          <div className="flex justify-between">
            ...
            <span className="font-medium text-gray-900">₹{(coupon.minOrderAmount / 100).toLocaleString('en-IN')}</span>
          </div>
        )}
        {coupon.validUntil && (
          <div className="flex justify-between">
