...
import { withMonitoring } from '@/lib/middleware/monitoring-middleware';
...
const searchQuerySchema = z.object({
  ...
  jobType: z.enum(['all', 'full-time', 'part-time', 'contract', 'internship']).optional().default('all'),
  experienceLevel: z.enum(['all', 'entry', 'mid', 'senior', 'lead', 'executive']).optional().default('all'),
  salaryMin: z.coerce.number().min(0).optional(),
  salaryMax: z.coerce.number().min(0).optional(),
  isRemote: z.coerce.boolean().optional().default(false),
  ...
  enableCache: z.coerce.boolean().optional().default(true)
});

async function enhancedJobsHandler(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    const filters: JobSearchFilters = {
      ...
      job_type: validationResult.data.jobType,
      ...
      remote_only: validationResult.data.isRemote,
      sector: validationResult.data.sector,
      skills: validationResult.data.skills,
      page: validationResult.data.page,
      limit: validationResult.data.limit,
      sort_by: validationResult.data.sortBy
    };
    ...
    const searchOptions = {
      enableCache: validationResult.data.enableCache,
      cacheTTL: 300, // 5 minutes
      maxResults: validationResult.data.limit * 2, // Get more results for better ranking
      includeExternal: validationResult.data.includeExternal,
      enableRanking: true
    };
    ...
    headers.set('X-Sources', metrics.sources.join(','));
...
export const GET = withMonitoring(enhancedJobsHandler);
