...
function sanitizeJobsResponse(jobs: Record<string, unknown>[]) {
  return jobs.map(job => ({
    id: job.id,
    title: job.title,
    company: job.company,
...
export async function GET(request: NextRequest) {let searchParams;
  try {
    searchParams = new URL(request.url).searchParams;const query = searchParams.get('query') || '';
    const location = searchParams.get('location') || 'India';
    const page = parseInt(searchParams.get('page') || '1');
    const limit = parseInt(searchParams.get('limit') || '20');// Use dynamic real API that works like Adzuna
    try {
