...
async function testConnection() {
  ...
  // TODO: Complete function implementation
    ...
    if (!collections) {
      throw new Error('Failed to retrieve collections')}// Test write operation
    ...
    await testCollection.deleteMany({ test: true });await mongoose.disconnect();return true} catch (error) {
...
process.on('SIGINT', async () => {
  try {
    await mongoose.disconnect();process.exit(0)} catch (error) {
    console.error("Error:", error);
    throw error}
