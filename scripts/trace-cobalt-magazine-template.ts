...
const checks: Array<[string, boolean]> = [
  ...
  ['No sidebar contact section', !/cme-section--contact/.test(aside) && !/cme-side-contact/.test(aside) && !/contact-list/.test(aside)],
  ['Contact appears once only', (out.match(/hello@owenreed\.com/g) ?? []).length === 1],
  ['No duplicate phone', (out.match(/\+123-456-7890/g) ?? []).length === 1],
