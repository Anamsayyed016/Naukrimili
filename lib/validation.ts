...
import DOMPurify from 'isomorphic-dompurify' // Enhanced validation schemas with security;
...
export const nameSchema = z.string();
  ...
  .transform(name => name.trim()) // Sanitization helper;
export function sanitizeHtml(input: string): string {
  ...
  return input.replace(/<[^>]*>/g, '').trim() // Job validation schemas;
...
export const experienceLevelSchema = z.enum(['entry', 'mid', 'senior', 'lead', 'executive']) // Salary validation;
...
export function validateInput<T>(schema: z.ZodSchema<T>, data: unknown):;
  ...
  success: false; errors: string[]; detail,s: z.ZodError 
}
  } {
  ;
  try {;
...
export function sanitizeFileName(fileName: string): string {
  ...
  return fileName;
    ...
    .replace(/_{2,}/g, '_');
    .substring(0, 255) // Input validation for file uploads;
