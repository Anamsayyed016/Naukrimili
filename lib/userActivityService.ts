...
class UserActivityService {
  ...
  getTrendingCategories(baseCategories: Array<{ name: string }>): TrendingMetrics[] {
    ...
    return Array.from(categoryMetrics.values())
      .map((metric) => ({
        ...metric,
        growth: metric.growth + metric.userRelevance * 5,
      }))
      .sort((a, b) => b.growth - a.growth);
  }

  // Get personalized job recommendations (filter presets)
  ...
  private analyzeSearchTimes(searchInteractions: UserInteraction[]) {
    ...
    const peakHour = this.getTopEntries(hourCounts, 1)[0] ?? 9; // default 9am
  ...
  cleanup() {
    ...
    this.interactions = this.interactions.filter((interaction) => interaction.timestamp >= cutoff);
    this.saveToStorage();
  }
...
export function useUserActivity() {
  ...
  const getTrendingCategories = (baseCategories: Array<{ name: string }>) =>
    userActivityService.getTrendingCategories(baseCategories);
  const getPersonalizedFilters = () => userActivityService.getPersonalizedFilters();
  const getSmartSuggestions = () => userActivityService.getSmartSuggestions();
  const getActivitySummary = () => userActivityService.getActivitySummary();
