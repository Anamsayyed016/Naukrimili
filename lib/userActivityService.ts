...
class UserActivityService {
  ...
  ;
    ...
    this.interactions.unshift(newInteraction) // Keep only recent interactions;
    if (this.interactions.length > this.MAX_INTERACTIONS) {
  ;
      this.interactions = this.interactions.slice(0, this.MAX_INTERACTIONS);
}
  }
    this.saveToStorage() // Get trending categories based on user activity;
  getTrendingCategories(baseCategories: Record<string, unknown>[]): TrendingMetrics[] {
  ;
    const categoryMetrics = new Map<string, TrendingMetrics>() // Initialize with base categories;
    baseCategories.forEach((cat, index) => { // Use deterministic growth based on category index to avoid hydration issues;
  ...
  ;
      if (interaction.type === 'search' || interaction.type === 'category_view') { // Find matching category;
    ...
    return Array.from(categoryMetrics.values()).map(metric => ({ ...metric }
      growth: metric.growth + (metric.userRelevance * 5), // User activity boosts growth
})).sort((a, b) => b.growth - a.growth) // Get personalized job recommendations;
  ...
  ;
    ...
    return patterns} // Get smart suggestions based on user behavior;
  getSmartSuggestions() {
...
export const userActivityService = new UserActivityService() // React hook for using the service;
