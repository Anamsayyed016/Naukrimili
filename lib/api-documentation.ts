...
export const AUTH_ENDPOINTS = {
   ...
   * @param role - User role ('jobseeker' | 'employer' | 'admin')
   * 
   * @returns {Object} Response object
   * @returns {string} Response.token - JWT access token
   * @returns {Object} Response.user - User profile data
   ...
   * 
   * @returns {Object} Response object
   * @returns {string} Response.token - New JWT access token
   */
 ...
 * Jobs Endpoints
 * 
 * Manage job postings and job-related operations
 */
...
export const JOBS_ENDPOINTS = {
   ...
   * @param search - Search query for job title/description
   * @param location - Filter by location
   * @param type - Filter by job type ('full-time' | 'part-time' | 'contract' | 'internship')
   * @param remote - Filter by remote work availability (true/false)
   * @param salary - Filter by salary range { min: number, max: number }
   * 
   * @returns {Object} Paginated response
   * @returns {Array} Response.data - Array of job objects
   * @returns {Object} Response.pagination - Pagination metadata
   * 
   * @example
   * const jobs = await fetch('/api/jobs?page=1&limit=20&search=developer&location=Mumbai');
   */
  LIST: '/api/jobs',
   ...
   * @param id - Job ID
   * 
   * @returns {Object} Job object with full details
   * 
   * @example
   * const job = await fetch('/api/jobs/64a1b2c3d4e5f6789012345');
   */
   ...
   * @param benefits - Array of job benefits
   * @param salary - Salary information { min: number, max: number, currency: string }
   * @param type - Job type
   * @param remote - Remote work availability
   * 
   ...
   * PUT /api/jobs/{id}
   * 
   * Update an existing job posting (Owner/Admin only)
   * 
   * @param id - Job ID
   ...
   * @param experience - Experience level filter
   * 
   ...
   * @param coverLetter - Cover letter text (optional)
   * 
...
export const APPLICATIONS_ENDPOINTS = {
   ...
   * 
   * Get user's job applications (paginated)
   * 
   * @param page - Page number
   * @param limit - Items per page
   * @param status - Filter by application status
   * 
   * @returns {Object} Paginated applications response
   */
 ...
 * Resumes Endpoints
 * 
 * Manage resume uploads and AI parsing
 */
...
export const RESUMES_ENDPOINTS = {
   ...
   * 
   * @param file - Resume file (PDF, DOCX, TXT)
   * @param jobId - Associated job ID (optional)
   * 
   * @returns {Object} Parsed resume data
   * @returns {Array} Response.skills - Extracted skills
   * @returns {Array} Response.experience - Work experience
   * @returns {number} Response.atsScore - ATS compatibility score
   */
   ...
   * @param id - Resume ID
   * 
   * @returns {Object} Resume object with parsed data
   */
...
export const USER_ENDPOINTS = {
   ...
   * @param experience - Work experience array
   * 
...
export const ADMIN_ENDPOINTS = {
   ...
   * 
   * @returns {Object} Paginated users response
   */
...
const { skills, experience, atsScore } = await response.json();
  `
};
...
export const RATE_LIMITS = {
  ...
  UPDATE_JOB: '20 requests per minute',
  
  // Application endpoints
  APPLY_JOB: '5 requests per minute',
  GET_APPLICATIONS: '50 requests per minute',
  
  // Resume endpoints
  UPLOAD_RESUME: '10 requests per minute',
  GET_RESUMES: '50 requests per minute',
...
export const DEPRECATED_ENDPOINTS = {
  ...
  // Old resume endpoint (use /api/resumes instead)
