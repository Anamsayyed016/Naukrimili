 ...
 * 
 * Note: Removed 'as const' assertions to avoid Next.js bundler TDZ issues
 */

// Education Levels / Degrees - Lazy initialization
let educationLevelsCache: Array<{ value: string; label: string }> | null = null;
export function getEducationLevels(): Array<{ value: string; label: string }> {
...
// Detailed Degree Types - Lazy initialization
let degreeTypesCache: Record<string, Array<{ value: string; label: string }>> | null = null;
export function getDegreeTypes(): Record<string, Array<{ value: string; label: string }>> {
  if (!degreeTypesCache) {
    degreeTypesCache = {
      'high-school': [
        { value: 'high-school', label: 'High School' },
  ...
  return degreeTypesCache;
}

// Top International Universities - Lazy initialization
let internationalUniversitiesCache: string[] | null = null;
export function getInternationalUniversities(): string[] {
  if (!internationalUniversitiesCache) {
    internationalUniversitiesCache = [
      'Harvard University',
  ...
  return internationalUniversitiesCache;
}

// Top Indian Universities - Lazy initialization
let indianUniversitiesCache: string[] | null = null;
export function getIndianUniversities(): string[] {
  if (!indianUniversitiesCache) {
    indianUniversitiesCache = [
      // IITs
  ...
  return indianUniversitiesCache;
}

// Online Platforms - Lazy initialization
let onlinePlatformsCache: string[] | null = null;
export function getOnlinePlatforms(): string[] {
  if (!onlinePlatformsCache) {
    onlinePlatformsCache = [
      'Coursera',
      'Udemy',
      'edX',
  ...
  return onlinePlatformsCache;
}

// Community Colleges (Sample) - Lazy initialization
let communityCollegesCache: string[] | null = null;
export function getCommunityColleges(): string[] {
  if (!communityCollegesCache) {
    communityCollegesCache = [
      'Community College',
      'Junior College',
      'City College',
      'State College',
...
export function getAllInstitutions(): string[] {
  if (!allInstitutionsCache) {
    ...
    const international = Array.from(getInternationalUniversities());
    const indian = Array.from(getIndianUniversities());
    const online = Array.from(getOnlinePlatforms());
    const community = Array.from(getCommunityColleges());
    allInstitutionsCache = international.concat(indian, online, community);
  }
  return allInstitutionsCache;
}

// Fields of Study - Categorized - Lazy initialization
let fieldsOfStudyCache: Record<string, string[]> | null = null;
export function getFieldsOfStudy(): Record<string, string[]> {
  if (!fieldsOfStudyCache) {
    fieldsOfStudyCache = {
  'Computer Science & IT': [
    'Computer Science',
  ...
  return fieldsOfStudyCache;
}

// Location Data - Countries - Lazy initialization
...
// Indian States - Lazy initialization
let indianStatesCache: string[] | null = null;
export function getIndianStates(): string[] {
  if (!indianStatesCache) {
    indianStatesCache = [
      'Andhra Pradesh',
      'Arunachal Pradesh',
      'Assam',
  ...
  return indianStatesCache;
}

// Major Indian Cities - Lazy initialization
let indianCitiesCache: string[] | null = null;
...
// Special Location Options - Lazy initialization
let specialLocationsCache: string[] | null = null;
export function getSpecialLocations(): string[] {
  if (!specialLocationsCache) {
    specialLocationsCache = [
      'Remote',
      'Online Program',
      'Distance Mode',
      'Hybrid',
...
export function getScoreFormats(): Array<{ value: string; label: string }> {
  if (!scoreFormatsCache) {
    scoreFormatsCache = [
  ...
  return scoreFormatsCache;
}

// Honors & Awards - Lazy initialization
let honorsAwardsCache: Array<{ value: string; label: string }> | null = null;
export function getHonorsAwards(): Array<{ value: string; label: string }> {
  if (!honorsAwardsCache) {
    honorsAwardsCache = [
