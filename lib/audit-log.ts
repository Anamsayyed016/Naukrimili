...
class AuditLogger {
  ...
  log(event: Omit<AuditEvent, 'timestamp'>): void {
    ...
    this.logs.push(auditEvent) // Keep only recent logs;
    if (this.logs.length > this.maxLogs) {
  ;
      this.logs = this.logs.slice(-this.maxLogs);
}
  } // Log to console in development;
    if (env.NODE_ENV === 'development') {} // Alert on critical events;
  ...
  ;
    console.error('CRITICAL SECURITY EVENT: ', event) // In production: send to monitoring service
}
}
  ...
  ;
    ...
    return this.logs.filter(log =>;
      log.action === action && !log.success &&;
      new Date(log.timestamp).getTime() > cutoff);
}
  }
export const auditLogger = new AuditLogger() // Security event types;
...
export function logAuthEvent(action: string;
  success: boolean;
  userId?: string,
  ip?: string);
