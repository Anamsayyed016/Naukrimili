...
export const jobApi = {
  ...
  async getJobs(params: {
    ...
    try {
      ...
      return await apiClient.getPaginated<Job>(`/api/jobs?${queryParams.toString()}`, page, limit);
    } catch (error) {
      throw new ApiError(
  ...
  async updateJob(id: string, jobData: Partial<Job>): Promise<Job> {
    try {
      return await apiClient.put<Job>(`/api/jobs/${id}`, jobData);
    } catch (error) {
      throw new ApiError(
  ...
  async searchJobs(query: string, location?: string): Promise<Job[]> {
    try {
      ...
      return await apiClient.get<Job[]>(`/api/jobs/search?${params.toString()}`);
    } catch (error) {
      throw new ApiError(
...
export const applicationApi = {
  ...
  async applyForJob(jobId: string, applicationData: Partial<Application>): Promise<Application> {
    try {
      return await apiClient.post<Application>(`/api/jobs/${jobId}/apply`, applicationData);
    } catch (error) {
      throw new ApiError(
  ...
  async updateApplication(id: string, status: string): Promise<Application> {
    try {
      return await apiClient.patch<Application>(`/api/applications/${id}`, { status });
    } catch (error) {
      throw new ApiError(
