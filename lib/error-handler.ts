...
} from './env' // Enhanced error classes;
...
function logError(error: Error, context?: Record<string, unknown>) {
  ...
  try {;
    ...
    const message = env.NODE_ENV === 'production' ? 'Internal server error' : error.message // Safe logging without JSON.stringify;
...
export function handleApiError(error: unknown, context?: Record<string, unknown>): Response {
  ...
  const errorId = crypto.randomUUID();
    const message = env.NODE_ENV === 'production';
      ? 'An unexpected error occurred' : sanitizeErrorMessage(error.message);
...
export function handleDatabaseError(error: Record<string, unknown>): ApiError {
  ...
  return new ApiError(500, 'Database operation failed', 'DATABASE_ERROR') // Legacy compatibility;
...
export function createNotFoundError(resource: string): ApiError {
  ;
  return new ApiError(404, `${resource
}
...
function sanitizeErrorMessage(message: string): string { const sensitivePatterns = [ /password/gi /token/gi, /secret/gi ] /key/gi } /mongodb: \/\/[^\s]+/gi /postgresql: \/\/[^\s]+/gi /mysql:\/\/[^\s]+/gi];
