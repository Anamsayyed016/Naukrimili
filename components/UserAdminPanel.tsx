...
import { useUsersApi, UserSummary } from '@/hooks/useUsersApi';
        ...
        <div className="flex items-center gap-2">
          ...
          {editId !== null && (
            <button
              ...
              onClick={() => { setEditId(null); setForm({ name: '', email: '' }); }}
            >
              Cancel
            </button>
          )}
          ...
          <li key={user.id} className="flex items-center justify-between border-b py-2">
            ...
            <div className="flex gap-2">
              <button
                ...
                onClick={() => { setEditId(user.id as number); setForm({ name: user.name || '', email: user.email || '' }); }}
              >
                Edit
              </button>
              <button className="text-red-600" onClick={() => deleteUser(user.id as number)}>Delete</button>
            </div>
          </li>
        ))}
      </ul>
