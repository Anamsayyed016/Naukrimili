...
export default function UnifiedAuthFlow({ onAuthSuccess }: UnifiedAuthFlowProps) {
  ...
  const handleGoogleAuth = async () => {
    ...
    try {
      ...
      if (result?.error) {
        ...
        const email = result.url?.split('email=')[1]?.split('&')[0] || '';
  ...
  // Welcome Step (Initial)
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 flex items-center justify-center p-4">
      <Card className="w-full max-w-md">
        <CardHeader className="text-center">
          <CardTitle className="text-2xl font-bold">Welcome to JobPortal</CardTitle>
          <CardDescription>
          ...
          <div className="space-y-4">
            ...
            <form onSubmit={handleEmailRegistration} className="space-y-4">
              ...
              <Button
                type="submit"
                className="w-full"
                disabled={isLoading || !userData.email || !userData.name}
              >
                {isLoading ? (
                  <>
                    <Loader2 className="mr-2 h-4 w-4 animate-spin" />
