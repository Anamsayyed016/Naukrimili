...
export default function ResumeAdminPanel() {
  const { resumes, loading, error, createResume, updateResume, deleteResume } = useResumesApi();
  const [form, setForm] = useState({ userId: '', fileUrl: '' });
  ...
  return (
    <div className="p-6 bg-white rounded shadow max-w-xl mx-auto mt-8">
      ...
      >
        ...
        <input className="border px-2 py-1 rounded" placeholder="File URL" value={form.fileUrl} onChange={e => setForm(f => ({ ...f, fileUrl: e.target.value }))} />
