...
export default function ApplicationAdminPanel() {
  const { applications, loading, error, createApplication, updateApplication, deleteApplication } = useApplicationsApi();
  const [form, setForm] = useState({ jobId: '', userId: '', status: '' });
  ...
  return (
    <div className="p-6 bg-white rounded shadow max-w-xl mx-auto mt-8">
      ...
      >
        <input className="border px-2 py-1 rounded" placeholder="Job ID" value={form.jobId} onChange={e => setForm(f => ({ ...f, jobId: e.target.value }))} />
        <input className="border px-2 py-1 rounded" placeholder="User ID" value={form.userId} onChange={e => setForm(f => ({ ...f, userId: e.target.value }))} />
        <input className="border px-2 py-1 rounded" placeholder="Status" value={form.status} onChange={e => setForm(f => ({ ...f, status: e.target.value }))} />
