  ...
  return (
    <Alert className={status.connected ? 'border-green-200 bg-green-50' : 'border-red-200 bg-red-50'}>
      ...
      <AlertDescription className="flex flex-col gap-2">
        <div className={status.connected ? 'text-green-800' : 'text-red-800'}>
          {status.message}
        </div>
        
        {status.testResults && status.connected && (
          <div className="text-green-700 text-sm">
            ✅ Found {status.testResults.jobCount} test jobs
            {status.testResults.sampleJob && (
              <div className="mt-1 text-xs text-green-600">
                Sample: "{status.testResults.sampleJob.title}" at {status.testResults.sampleJob.company}
              </div>
            )}
          </div>
        )}
        ...
        {!status.connected && (
          <div className="text-xs text-red-600 mt-1">
            Check your SERPAPI_KEY environment variable and internet connection
          </div>
        )}
      </AlertDescription>
    </Alert>
