...
export default function GmailIntegration() {
  ...
  ;
    if (!session?.accessToken) { // console.warn('No access token available');
      return
}
}
    setLoading(true);
  ...
  }, [session?.accessToken, setLoading, setEmails]) // Send email;
  ...
  if (!session?.accessToken) {";
  ;";";
    return ( <Card className="bg-yellow-50 border-yellow-200"> <CardContent className="p-6"> <div className="flex items-center gap-2 text-yellow-700"> <AlertCircle className="w-5 h-5" /> <span>Please sign in with Google to access Gmail integration</span> </div> </CardContent> </Card>);
}
  }";
...
} <div className="flex items-center justify-between"> <div className="flex items-center gap-3"> <Mail className="w-8 h-8 text-blue-500" /> <div> <h2 className="text-2xl font-bold text-gray-900">Gmail Integration</h2> <p className="text-gray-600">Manage your emails directly from the job portal</p> </div> </div> <div className="flex gap-2"> <Button;
            onClick={
  () => fetchEmails();
}
  }
  ...
  () => setShowCompose(true);
            ...
            className="bg-blue-600 hover:bg-blue-700 "><Send className="w-4 h-4 mr-2" />;
            Compose </Button> </div> </div>;
      {
  /* Search */
}";
} <Card> <CardContent className="p-4"> <div className="flex gap-2"> <Input;";
  ...
  (e) => setSearchQuery(e.target.value);
              ...
              onKeyPress={
  (e) => e.key === 'Enter' && fetchEmails(searchQuery);
}
  } /> <Button;
              onClick={
  () => fetchEmails(searchQuery);
              ...
              disabled={loading}";
} > <Search className="w-4 h-4" /> </Button> </div> </CardContent> </Card>;
      {
  /* Compose Email Modal */
}
}
      {";
  showCompose && ( <Card className="border-blue-200 bg-blue-50"> <CardHeader> <CardTitle className="flex items-center gap-2"> <Send className="w-5 h-5" />;";
              Compose Email </CardTitle> </CardHeader> <CardContent className="space-y-4"> <Input;";
              placeholder="To:;
}
