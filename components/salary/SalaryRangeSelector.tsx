...
import {
  currencyMap, getCurrencyInfo, CurrencyInfo
}";
} from "@/lib/currencyMap";
...
interface SalaryRangeSelectorProps {
  ...
  value: SalaryRange;
  onChange: (rang,e: SalaryRange) => void
}
}
}
const SalaryRangeSelector: React.FC<SalaryRangeSelectorProps> = ({
  ...
  const [max, setMax] = useState(value.max || currency.max) // Update currency on country change;
  useEffect(() => {
...
}, [countryCode]) // Handle period toggle;";
  ...
  ;
    ...
    setMax(newMax);
    onChange({
  min: newMin, max: newMax, currency: currency.code, period: p
}
  ...
  return ( <div style={{ maxWidth: 500, margin: "0 auto 32px", background: "#fff", borderRadius: 16, boxShadow: "0 2px 12px #0001", padding: 24 }";
  ...
  {";
            ...
            Annual </button> <button;
            onClick={";
  () => handlePeriodToggle("month");
}
  }
            style={
  {";
            ...
            Monthly </button> </div> </div> <div style={{ margin: "32px 0 16px" }
}> <input;";
          type="range;
          min={currency.min}
}
          ...
          onChange={
  e => handleSlider(snap(Number(e.target.value)), max);
}
  }";
          style={{ width: "100%" }
}
          aria-label={
  `Minimum salary: ${currency.format(min);
}
  }`} /> <input;";
          type="range;
          min={currency.min}
          ...
          onChange={
  e => handleSlider(min, snap(Number(e.target.value)));
}
  }";
          style={{ width: "100%", marginTop: 8 }
}
          aria-label={
  `Maximum salary: ${currency.format(max);
}";
  ...
  currency.format(currency.min);
}
  }</span> <span>{
  currency.format(currency.max);
}";
  }</span> </div> </div> <div style={{ fontSize: 13, color: "#6b7280", marginTop: 8 }
}> <span title={`Salaries shown in ${currency.symbol}
} based on ${
